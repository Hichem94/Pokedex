...
class PokedexScreen(Screen):
    def __init__(self, **kwargs):
        ...
        scroll_view = ScrollView(size_hint=(1, 0.8), pos_hint={'x': 0, 'y': 0.035})

        # Créer la grille pour afficher les Pokémons
        self.grid = GridLayout(cols=1, padding=[20, 20, 20, 65], spacing=[15, 15], size_hint_y=None)
        self.grid.bind(minimum_height=self.grid.setter('height'))

        for index, pokemon in enumerate(mypokemons):
            ...
            pokemon_type_for_icons = pokemon['types']

            # Pour les images des pokemons
            try:
                # Télécharger l'image
